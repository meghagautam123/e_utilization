...
import 'package:e_utilization/utils/TextFormFieldStyle.dart';
...
class EFormCamera extends StatefulWidget {
  ...
  String? username;
  String? token;
  List<Map<String, double>>? locationListnew;
  ...
  EFormCamera.fromScreen(Data userData, String? username, String? token, List<Map<String, double>> locationList )
  {
    ...
    this.username = username;
    this.token = token;
     this.locationListnew = locationList;
  }
...
class _EFormCameraState extends State<EFormCamera> {
  ...
  final int maxLength =30; // Define your character limit here
  int remainingChars = 30;
  String? deviceId;
...
*/
  ...
  void initState() {
    ...
    remarkController.addListener(_updateRemainingChars);
    _getDeviceInfo();
    getName();
  ...
  void dispose() {
    _controller.dispose();
    remarkController.removeListener(_updateRemainingChars);
  ...
  void _updateRemainingChars() {
    setState(() {
      remainingChars = maxLength - remarkController.text.length;
    });
  }
...
//if(_imageList.length)
  ...
  void getCurrentPosition() async
  {
    ...
    else
    {
     /*Position currentPosition = await Geolocator.getCurrentPosition(
      ...
      final locationSettings = LocationSettings(accuracy: LocationAccuracy.high, distanceFilter: 20);
       positionStream = Geolocator.getPositionStream(locationSettings: locationSettings).listen(
              (Position position)
           {
            ...
            Fluttertoast.showToast(
                msg: '' + position.longitude.toString(),
                toastLength: Toast.LENGTH_SHORT,
                gravity: ToastGravity.BOTTOM,
                timeInSecForIosWeb: 1,
                backgroundColor: Colors.red,
  ...
  Future<void> sendRequest(int i, String username, String photonew,
     ...
     if(status == "Success")
    {
     ...
     //  showdialog(context,successmessage);
      showDialog(
        context: context,
        builder: (BuildContext context) => CommonDialog.buildPopup(context,successmessage),
  ...
  else
    {
      ...
      showDialog(
        context: context,
        builder: (BuildContext context) => CommonDialog.buildPopup(context,failedmessage),
      );
    ...
    } catch (e)
    {
      ...
      showDialog(
        context: context,
        builder: (BuildContext context) => CommonDialog.buildPopup(context,e.toString()),
      );
  ...
  Widget _buildPopupDialog(BuildContext context,String successmessage) {
  ...
  void _startLoading()
  {
   ...
   // String deviceId = androidInfo.androidId;
    ...
    print("deviceid123"+deviceId!);

    sendRequest(i, stringValue, _base64Image!, latitude,
        longitude, deviceId!);
  }

  void showdialog(BuildContext context, String successmessage) {}
